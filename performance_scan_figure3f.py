...
for i in np.arange(len_kz):
    ...
    # where to save
    filename = "data_raw/figure3f/sig="+str(sigmaN)+"_k="+str(kappa_star)+"_beta="+str(beta)
    ...
    for j in range(0, nIter): # run for nIter iterations
        ...
        # network filter, no noise
        N = 64
        r, mu, kappa = flt.RNN_filter_Run(T,N=N,w_even=w_even,w_odd=w_odd,tau=tau,w_quad=w_quad,
                                I_ext=I_ext,stoch_corr=stoch_corr,z=z,dy=dy,phi_0=phi_0,kappa_0=kappa_0,dt=dt,sigma_N=sigmaN)
